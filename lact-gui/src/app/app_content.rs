...
use gtk::{glib::{self, clone, ControlFlow}, prelude::{BoxExt, Cast, DialogExtManual, FileChooserExt, FileExt, GtkWindowExt, OrientableExt, WidgetExt}, ButtonsType, FileChooserAction, FileChooserDialog, MessageDialog, MessageType, ResponseType};
use i18n_embed_fl::fl;
use lact_client::DaemonClient;
use lact_schema::{request::{ConfirmCommand, ProfileBase, SetClocksCommand}, DeviceStats, config::{GpuConfig, Profile}, DeviceListEntry, ClocksTable, PowerStates, ProfilesInfo, DeviceInfo, SystemInfo};
use amdgpu_sysfs::gpu_handle::power_profile_mode::PowerProfileModesTable;
use relm4::{AsyncComponentSender, Component, ComponentController, RelmObjectExt, binding::BoolBinding, prelude::{AsyncComponent, AsyncComponentParts}, tokio::{self, time::sleep}};
use relm4_components::{open_dialog::{OpenDialog, OpenDialogMsg, OpenDialogResponse, OpenDialogSettings}, save_dialog::{SaveDialog, SaveDialogMsg, SaveDialogResponse, SaveDialogSettings}};
use std::{fs, path::PathBuf, rc::Rc, sync::{Arc, atomic::AtomicBool}, time::Duration};
use tracing::{debug, error, trace, warn};

use super::{apply_revealer::{ApplyRevealer, ApplyRevealerMsg}, confirmation_dialog::ConfirmationDialog, ext::RelmDefaultLauchable, graphs_window::{GraphsWindow, GraphsWindowMsg}, header::{Header, HeaderMsg, profile_rule_window::{ProfileRuleWindowMsg, profile_row::ProfileRuleRowMsg}}, pages::{PageUpdate, crash_page::CrashPage, info_page::InformationPage, oc_page::{OcPage, OcPageMsg}, software_page::{SoftwarePage, SoftwarePageMsg}, thermals_page::{ThermalsPage, ThermalsPageMsg}}, msg::AppMsg, show_error, show_embedded_info};
...
pub struct AppContent {
    ...
    pub stats_task_handle: Option<glib::JoinHandle<()>>,
    pub root: gtk::ApplicationWindow,
}
...
impl AsyncComponent for AppContent {
    ...
    async fn init(
        ...
        if let Some(err) = conn_err {
            show_embedded_info(&model.root, anyhow!(err.to_string()));
        }
        ...
        if let Some((gpu_id, gpu_data)) = initial_gpu {
            ...
            model.stats_task_handle = Some(start_stats_update_loop(
                gpu_id,
                model.daemon_client.clone(),
                sender.clone(),
                model.header.sender().clone(),
            ));
        } else {
...
impl AppContent {
    async fn handle_msg(
        ...
        match msg {
            ...
            AppMsg::ApplyChanges => {
                self.apply_settings(self.current_gpu_id()?, &self.root, &sender)
                    .await
                    .inspect_err(|_| {
                        sender.input(AppMsg::ReloadData { full: false });
                    })?;
            ...
            AppMsg::DumpVBios => {
                self.dump_vbios(&self.current_gpu_id()?, &self.root).await;
            }
            AppMsg::DebugSnapshot => {
                self.generate_debug_snapshot(&self.root).await;
            }
            ...
            AppMsg::EvaluateProfile(rule, profile_sender) => {
                match self.daemon_client.evaluate_profile_rule(rule).await {
                    Ok(matches) => {
                        profile_sender.emit(ProfileRuleWindowMsg::EvaluationResult(matches));
                    }
                    Err(err) => {
                        warn!("{err:#}");
                    }
            ...
            AppMsg::DataLoaded { .. } | AppMsg::LoadingStatus(_) => {
            }
        }
        Ok(())
    }
